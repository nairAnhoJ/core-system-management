            ...
            <tbody>
                @foreach ($results as $result)
                    <tr wire:key="model-{{ $result->id }}" class="text-center bg-gray-100 even:bg-gray-200">
                        ...
                        <td>
                            <livewire:departments.edit-department :id="$result->id" :wire:key="'edit-'.$result->id">
                            <span class="mx-[1px]">|</span>
                            <livewire:departments.delete-department :id="$result->id" :wire:key="'delete-'.$result->id">
                        </td>
                    </tr>
                @endforeach
            </tbody>
